...
class HyponymyScoreLossLayer(unittest.TestCase):
    ...
    def test_soft_common_prefix_length(self):
        ...
        expected = np.array([utils.calc_soft_common_prefix_length(mat_x, mat_y) for mat_x, mat_y in zip(arry_test_x, arry_test_y)])
        actual = self._loss_layer.calc_soft_common_prefix_length(t_test_x, t_test_y).data.numpy()
